...
def get_parameters(session_parameters, system_id, applied_rules):
    # добавляет в сессию параметры, пока находятся новые
    once_more = True
    while once_more:
        new_parameters = {}
        results = scan_rules(session_parameters, Rule.PARAM_RULE, system_id, applied_rules)
...
def get_attributes(session_parameters, system_id, applied_rules):
    # возвращает атрибуты
    attrs = {}
    results = scan_rules(session_parameters, Rule.ATTR_RULE, system_id, applied_rules)
    for result in results:
...
def scan_rules(param_dict, type, system_id, applied_rules):
    # проверяет все правила и возвращает результат
    rules = Rule.objects.filter(type=type, system_id=system_id)
    results = []
    for rule in rules:
        if rule.id not in applied_rules:
            ...
            for literal in literals:
                values_from_map = param_dict.get(str(literal['param']), [])
                if not values_from_map:
